...
const Navbar = () => {
  ...
  // Function to navigate to delivery form
  ...
  // Conditionally render the Navbar only if the user is not in a specific role
  if (user && (user.role === 'GESTIONNAIRE' || user.role === 'ADMIN' || user.role === 'LIVREUR')) {
    return null; // Do not render the Navbar if user has these roles
  }
    ...
    <nav className="bg-[#e8f7f9] py-3 px-6 shadow-lg relative z-10">
      <div className="container mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6">
          <Link to="/" className="text-darkCyan font-medium hover:text-primary">Accueil</Link>
          <Link to="/About" className="text-darkCyan font-medium hover:text-primary">À propos</Link>
          <button
            className="border-2 border-primary text-primary rounded-full px-4 py-1 font-medium hover:bg-primary hover:text-white transition"
